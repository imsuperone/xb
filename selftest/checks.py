...
try:
    ...
    from ..engines import slave as _slave
    from ..engines import slave_text as _T
except ImportError:
    import store as ST
    try:
        from engines import slave as _slave
...
def _cur_spirits(gid, qq):
    try:
        return _json.loads(ST.acct(gid, qq).get("spirits", "") or "{}")
    except Exception:
        return {}
...
def _cur_rides(gid, qq):
    try:
        return _json.loads(ST.acct(gid, qq).get("rides", "") or "{}")
    except Exception:
        return {}
