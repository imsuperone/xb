...
- `指令启用配置` 206 开关（默认真=行为不变，v0.7.26 起空格变体合并规范键，匹配空格无关；v0.7.27 去超管别名/噪音；v0.7.29 删 9 查询转发，`slave._QUERY_SILENT` 变量引用刻意避开采集正则）；`指令回复配置` 206 场景（默认空=内置回复）；`指令权限配置` 206 位（默认所有人，27 键默认超管，v0.7.29 起；删 9 查询前为 30 键）；`_collect_commands` 为唯一采集源，改引擎指令写法后重跑播种脚本（`Temp/opencode/seed_schema_075.py` 思路）。
...
- **徽标与漂移**：徽标禁写死，`refreshBalanceBadges()` 读 `config/balance_state`（12 签名键抽检，v0.7.34 起剔除混合态的抽武器花费）；偏离置灰点名；徽标可点直达弹窗。**推断规则（v0.7.31 起）**：逐档算 mismatch 取最小，缺键=未自定义不计入，并列优先标记档其次 standard；出厂种子≈休闲数值故默认命中休闲档，应用某档后命中该档，手调才报真偏离。弹窗预选 `state.mode` 而非标记值。
...
| 页/区 | 操作 | 写什么 |
| :--- | :--- | :--- |
| 商城·坐骑 | 保存坐骑/恢复默认（顶部同栏） | `商城图鉴.ride_shop` |
| 商城·抽奖池 | 改名/稀有度/删/传/换图 | 池文件（即时） |
| 商城·抽奖池 | 保存武器属性/恢复默认（顶部同栏） | `商城图鉴.weapon_attrs`（恢复=硬清） |
| 商城·抽奖池 | ↑↓ 排序（商城池行内，v0.7.33 起，v0.7.36 起图鉴武器页签已删） | `商城图鉴.weapon_order`（即时） |
| 商城·道具 | 保存道具/恢复默认（顶部同栏） | 精灵图鉴 `shop`（恢复=硬清即时） |
| 商城·顶部全部保存（v0.7.31） | 一键存三栏 | 上三项各存各的（宝物/精灵地图不动） |
| 总览·宝物/坐骑 | 同商城对应能力 | 同上（宝物名+效果即时；武器只在商城，v0.7.36 起不在图鉴展示） |
| 总览·精灵 | 保存地图/属性/恢复 | 精灵图鉴 `maps`/`spirits`（恢复=硬清即时） |
| 总览·精灵 | 清空地图/地图排序/精灵改名（v0.7.33） | `maps` 清空留精灵；改名同步掉落+进化引用 |
| 必要配置·恢复 | 仅本页已渲染节 + 二次确认 | 对应节 |
| 必要配置·全部设置恢复默认（v0.7.29，红线 9 唯一例外） | 跨数值/开关节 + 二次确认，排除备份配置/商城图鉴/精灵图鉴/自定义指令/群组开关，不动用户数据，依赖自动快照回滚 | schema 默认（排除项原样保留） |
...
## 7. WebUI 要点（app.js，~5450 行）

- Bridge 优先 + fetch 回退；`uiConfirm/uiPrompt/uiAlert`（iframe 下禁原生弹窗）；按钮忙态锁。
- 回执只说结果不 dump JSON；危险操作（删文件/删图/清属性/恢复默认）一律二次确认；空列表是合法结果，`callApi` 不再以空回退 POST（v0.7.29，双倍请求修复）。
- 商城三栏标题只留“X商城 — N 件”（v0.7.29 去英文键名与介绍后缀）；保存/恢复与添加同栏置顶（v0.7.31）；收服率文案去“大师球必中”后缀。
- 指令页双接口并行 + `#cmdList` 事件委托一次绑定 + 保存后恢复展开态（v0.7.31）。
- 安装包排除顶层文档与 `scripts/`（v0.7.31；仓库保留，`verify_plugin` 照常校验 9 处）。
...
| 版本 | 主题 |
| :--- | :--- |
| v0.7.36 | 图鉴去武器页签（宝物保留）；指令数值去乱配 |
| v0.7.35 | 精灵改名行内化（坐骑同款） |
| v0.7.34 | 删金字塔/24h假曲线；漂移抽检12键；导出统一兜底 |
| v0.7.33 | 清空精灵地图；坐骑/武器/地图排序；精灵改名；武器顺序键 |
| v0.7.32 | 红包降级超发修复；删除按钮复活；5 键补回；进监狱 8 次；小清理 |
| v0.7.31 | 漂移误报根治（逐档推断+缺键豁免）；指令页提速；商城布局上移+全部保存；安装包去文档 |
| v0.7.30 | 重接口异步化；日志减负；更新检测收敛；在途共享；打劫有界；帮派缓存15秒 |
| v0.7.29 | 删9查询转发；必要配置全部恢复默认；商城标题精简；空投回滚/红包/死代码清理 |
| v0.7.28 | 备份防重收敛；数值精准匹配；商城图鉴提速 |
