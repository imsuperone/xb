...
| 总览·图鉴 | 宝物/精灵（v0.7.37 起无坐骑，坐骑只在商城） | 同上 |
| 商城·抽奖池 | 改名/稀有度/删/传/换图 | 池文件（即时） |
| 商城·抽奖池 | 保存武器属性/恢复默认（顶部同栏） | `商城图鉴.weapon_attrs`（恢复=硬清） |
| 商城·抽奖池 | ↑↓ 排序（商城池行内） | `商城图鉴.weapon_order`（即时） |
| 商城·大类排序（v0.7.37） | 坐骑/武器池/道具三栏调序 | `商城图鉴.shop_order`（即时） |
| 商城·导出导入（v0.7.37） | 全量包：坐骑/武器属性+顺序+清单/精灵道具 | 同上各写各的（缺图跳过提示） |
| 商城·道具 | 保存道具/恢复默认（顶部同栏） | 精灵图鉴 `shop`（恢复=硬清即时） |
| 商城·顶部全部保存（v0.7.31） | 一键存三栏 | 上三项各存各的（宝物/精灵地图不动） |
| 总览·宝物/精灵 | 同商城对应能力 | 同上（武器坐骑只在商城，v0.7.36/37 起不在图鉴展示） |
...
| 版本 | 主题 |
| :--- | :--- |
| v0.7.37 | 删补偿/图鉴坐骑/刷新钮；快照防堆积；商城全量导出；大类排序；回到顶部；滑动提速 |
