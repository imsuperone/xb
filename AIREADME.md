...
- **徽标与漂移**：徽标禁写死，`refreshBalanceBadges()` 读 `config/balance_state`（13 签名键抽检）；偏离置灰点名；徽标可点直达弹窗。**推断规则（v0.7.31 起）**：逐档算 mismatch 取最小，缺键=未自定义不计入，并列优先标记档其次 standard；出厂种子≈休闲数值故默认命中休闲档，应用某档后命中该档，手调才报真偏离。弹窗预选 `state.mode` 而非标记值。
