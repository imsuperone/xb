...
| **当前版本** | `v0.7.2` (已发布并在 GitHub 与本地保持对齐) |
...
| **离线发行包** | `C:\Users\Light\Desktop\DRxb\astrbot_plugin_xbbot_v0.7.2.zip` |
| **平台依赖** | AstrBot >= 3.4.0, Python >= 3.10 (支持 3.14t/自由线程) |
| **底层协议端** | OneBot v11 (aiocqhttp), NapCat, Lagrange 原生图文 |

---

## 🚦 系统运行状态与核心机制指标 (至 v0.7.2)

- **Python 模块语法解析**：41/41 语法解析通过 (100%)。
- **版本号命名与迭代规范 (0.7.xx 规范)**：当前基准版本 `0.7.2`，严格遵循 `0.7.xx`（`0.7.0`~`0.7.99`）后置补丁递增规则；仅在补丁位满 99 或用户明确要求时才更迭前段大版本（`0.8.xx`/`1.0.xx`）。
...
- **彻底根除双份备份 (v0.7.0)**：
  - 5 秒全局备份防抖与路径复用，杜绝多次点击与上传重叠生成两份冗余备份。
